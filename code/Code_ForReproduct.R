...
# getwd()
# setwd("/Users/utility/Desktop/R_For_Dissertation/Probabilistic-Forecasting-for-Curves-master/code")
# getwd()
...
DataGeneration = function(N, Ns, D, Nu, sigmaE, HO, Alpha, p=1, dist = 'norm', sparsity = FALSE){
  ...
  U = seq(-1, 1, 2/(Nu -1))
  Y = matrix(nrow = N + Ns, ncol = Nu) 
  XI = matrix(nrow = BurnIn + N + Ns + p, ncol = D) 
  PhiT = matrix(nrow = D, ncol = Nu) 
...
# df1 <-readRDS(file="result_groupbymonth_1-30&71-100.RData")
# df11 <-readRDS(file="result_innormal_1-30&71-100.RData")
# df2 <- df1[1:720,]
# df22 <- df11[1:60,]
# df3 <-readRDS(file="result_groupbymonth_31-70.RData")
# df33 <-readRDS(file="result_innormal_31-70.RData")
# 
# 
...
write.csv(result_groupbymonth, "Result_groupbymonth.csv")
write.csv(result_innormal, "Result_innormal.csv")
